...
class Plotter:
    def __init__(self):
        ...
        plt.xticks([0,1,2,3,4,5,6,7],labels=["X pos","Y pos","X vel","Y vel","Ang","A vel","L leg","R leg"],fontsize="small",rotation=0)
        self.gaussIn = plt.imshow(np.random.rand(8,64).T, aspect=0.1, interpolation='nearest', norm=colors.SymLogNorm(linthresh=0.0003, linscale=0.003,
                                              vmin=0.0, vmax=1.0, base=10))
        ax0 = plt.subplot(2, 3, 2)
        plt.title("1st Layer Spikes")
        plt.xlabel("Simulation time")
        plt.ylabel("Neuron ID")
        self.line0 = plt.imshow(np.random.rand(16, 128).T, aspect=0.3, interpolation='nearest')
        ...
        self.line1 = plt.imshow(np.random.rand(16, 128).T, aspect=0.1, interpolation='nearest',norm=colors.Normalize(
                                              vmin=0.0, vmax=3.0))
        ...
        plt.xlabel("Actions")
        self.bar = plt.bar(["Do nothing", "Right engine", "Main engine", "Left engine"], [0, 0, 0, 0])
        ...
        plt.legend(["Do nothing", "Right engine", "Main engine", "Left engine"],loc="upper left")
    ...
    def update(self, *args):
        try:
            ...
            pass
        return [self.line0, self.line1, *self.bar, *self.plotlines2,self.gaussIn, self.envrender]#, *self.plotlines1]
